...
async def save_uploaded_photo(request: Request, current_photo: str = "") -> str:
    ...
    suffix = Path(Path(photo.filename).name).suffix.lower()
...
def render_service(category_name: str, service_name: str, service_data: object) -> str:
    ...
    if photo:
        photo_block = f"""
            <div class="photo-line">
              <span>Фото:</span>
              <a href="/uploads/{esc(Path(photo).name)}" target="_blank">{esc(Path(photo).name)}</a>
            </div>
            <form method="post" action="/service/photo/delete" onsubmit="return confirm('Удалить фото услуги?')">
              ...
              <button class="danger secondary-action" type="submit">Удалить фото</button>
            </form>
        """
    else:
        photo_block = '<p class="muted">Фото не добавлено.</p>'
    ...
    <article class="service">
      ...
      <form method="post" action="/service/delete" onsubmit="return confirm('Удалить услугу?')">
        <input type="hidden" name="category" value="{esc(category_name)}">
        <input type="hidden" name="name" value="{esc(service_name)}">
        <button class="danger secondary-action" type="submit">Удалить услугу</button>
      </form>
    </article>
    """


def render_action(index: int, action: dict[str, str]) -> str:
    return f"""
    <article class="service">
      ...
      <form method="post" action="/action/delete" onsubmit="return confirm('Удалить функцию?')">
        <input type="hidden" name="index" value="{index}">
        <button class="danger secondary-action" type="submit">Удалить функцию</button>
      </form>
    </article>
    """
...
def index() -> str:
    ...
    for category_name, services in catalog.items():
        services_html = "".join(
            render_service(category_name, service_name, service_data)
        ...
        category_sections.append(
            ...
            <section class="panel">
              ...
              {services_html or '<p class="muted">В категории пока нет услуг.</p>'}
            </section>
            """
        )

    actions_html = "".join(render_action(index, action) for index, action in enumerate(actions))
    ...
    <head>
      ...
      <style>
        ...
        .muted, .photo-line {{ color: #667085; }}
        .photo-line {{ display: flex; gap: 6px; margin-top: 12px; }}
    ...
    <body>
      <header>
        ...
        <p>Каталог, фото услуг и простые функции главного меню</p>
      </header>
      <main>
        <div class="grid">
          <section class="panel">
        ...
        <section class="panel">
          ...
          {actions_html or '<p class="muted">Дополнительных функций пока нет.</p>'}
        </section>
      </main>
    </body>
    </html>
