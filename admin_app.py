...
from catalog import get_catalog, save_catalog
      ...
      <main>
        <div class="grid">
          ...
          <section class="panel">
            ...
            <form method="post" action="/service/add" class="stack">
              <label>Категория
...
async def add_service(request: Request) -> RedirectResponse:
    ...
    if category and name and description:
        catalog = get_catalog()
        catalog.setdefault(category, {})[name] = description
        save_catalog(catalog)
    return redirect_home()
...
async def update_service(request: Request) -> RedirectResponse:
    ...
    if category in catalog and old_name in catalog[category] and new_name and description:
        ...
        catalog[category][new_name] = description
        save_catalog(catalog)
    return redirect_home()
